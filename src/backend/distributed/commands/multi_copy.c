...
CopyToExistingShards(CopyStmt *copyStatement, char *completionTag)
{
	 ...
	 * unti the object is parsed by the worker, which is unable to give an accurate
	 * line number.
	 */
	if (SkipJsonbValidationInCopy && !isInputFormatBinary)
	{
		...
		foreach(jsonbColumnIndexCell, jsonbColumnIndexList)
		{
			...
			FmgrInfo *currentOutputFunction =
			...
			if (copyOutState->binary)
			{
				 ...
				 * is a version number followed by text. We thefere use an output
				 * function which sends the text as if it were jsonb, namely by
				 * prepending a version number.
				 */
				fmgr_info(textSendAsJsonbFunctionId, currentOutputFunction);
			}
		}
	}
	...
	for (columnIndex = 0; columnIndex < columnCount; columnIndex++)
	{
		...
		if (inputColumnNameList != NIL)
		{
			...
			foreach(inputColumnCell, inputColumnNameList)
			{
				...
				if (strncmp(inputColumnName, columnName, NAMEDATALEN) == 0)
				{
					isInputColumn = true;
					break;
				}
